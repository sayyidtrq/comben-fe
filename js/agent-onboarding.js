(() => {
  ...
  function updateChecklist(scope, countSelector, progressSelector) {
    ...
    checks.forEach((item) => {
      ...
      if (!em) return;
      if (scope.includes("training")) {
        em.textContent = item.checked ? "Selesai 22 Mei 2025" : "Belum selesai";
      } else {
        em.textContent = item.checked ? "Selesai" : "Belum diunggah";
      }
    });
  ...
  function updateSummary() {
    const allChecks = Array.from(document.querySelectorAll('.check-list input[type="checkbox"]'));
  ...
  document.querySelector("[data-save-draft]")?.addEventListener("click", () => {
    showToast("Draft onboarding berhasil disimpan.");
  });
